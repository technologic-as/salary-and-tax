import { defaultSalaryCalculationValues } from '../../../reducers/calculations';
import {
  getCompanyIncome, getEmployerFee, getPension, getSalaryCalculations, getTurnover, getVacationSavings,
} from '../calculations';
...
describe('calculations', () => {
  ...
  describe('getEmployerFee', () => {
    it('should calculate the employer fee when included', () => {
      expect(getEmployerFee(expectedAfterCut, {includeEmployerFee: true, employerFeeRate: 14.1})).toEqual({
        after: expectedAfterEmployerFee,
        employerFee: 211648,
      });
    });
    it('should not calculate the employer fee when not included', () => {
      expect(getEmployerFee(expectedAfterCut, {includeEmployerFee: false, employerFeeRate: 14.1})).toEqual({
        after: expectedAfterCut,
        employerFee: 0,
      });
    });
  ...
  describe('getVacationSavings', () => {
    it('should calculate the vacation savings when included', () => {
      expect(getVacationSavings(expectedAfterEmployerFee, {includeVacationSavings: true, vacationSavingsRate: 12.5})).toEqual({
        after: expectedAfterVacationSavings,
        vacationSavings: 166783,
      });
    });
    it('should not calculate the vacation savings when not included', () => {
      expect(getVacationSavings(expectedAfterEmployerFee, {includeVacationSavings: false, vacationSavingsRate: 12.5})).toEqual({
        after: expectedAfterEmployerFee,
        vacationSavings: 0,
      });
    });
  ...
  describe('getPension', () => {
    ...
    it('should not calculate while not included', () => {
      expect(getPension(pensionData.oneG * 5, {...pensionData, includePension: false})).toEqual({
        after: pensionData.oneG * 5,
        pension: 0,
      });
      expect(getPension(pensionData.oneG * 7, {...pensionData, includePension: false})).toEqual({
        after: pensionData.oneG * 7,
        pension: 0,
      });
      expect(getPension(pensionData.oneG * 12, {...pensionData, includePension: false})).toEqual({
        after: pensionData.oneG * 12,
        pension: 0,
      });
      expect(getPension(pensionData.oneG * 13, {...pensionData, includePension: false})).toEqual({
