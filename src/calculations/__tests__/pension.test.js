  ...
  tests.map((g) => {
    ...
    describe(`given salary of ${salary} (${g}G)`, () => {
      ...
      it('should not calculate if include is false', () => {
        const noPensionData = {pension: {...pensionConstants.pension, include: false}};
      ...
      it('step 1 should never be greater than 6% of 7.1-1 G', () => {
        expect(calculateStep1(salary, pensionConstants.pension)).not.toBeGreaterThan(35459);
      });
      it('step 2 should never be greater than 6% of 12-7.1 G', () => {
        expect(calculateStep2(salary, pensionConstants.pension)).not.toBeGreaterThan(28483);
      });
      it('getPension should never be greater than 6% of 12-1 G', () => {
        expect(getPension(salary, pensionConstants).pension).not.toBeGreaterThan(63942);
      });
      if (g <= 7.1) {
