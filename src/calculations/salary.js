...
export const getEmployerFee = (salary, { employerFee: { include, rate } }) => {
  const employerFee = round(
    salary - (salary / (100 + (include ? parseFloat(rate) : 0))) * 100
  );
  return { employerFee, after: salary - employerFee };
};
export const getVacationSavings = (
  salary,
  { vacationSavings: { include, rate } }
) => {
  const vacationSavings = round(
    salary - (salary / (100 + (include ? parseFloat(rate) : 0))) * 100
  );
...
export const getSalaryCalculations = data => {
  ...
  const { surplus, after: withoutSurplus } = calculateSurplus(
    companyIncome,
    data
  );
  const { employerFee, after: withoutEmployerFee } = getEmployerFee(
    withoutSurplus,
    data
  );
  const { vacationSavings, after: withoutVacationSavings } = getVacationSavings(
    withoutEmployerFee,
    data
  );
  const { pension, after: withoutPension } = getPension(
