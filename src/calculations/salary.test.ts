...
describe('salary', () => {
  ...
  describe('getTurnover', () => {
    ...
    it('should calculate turnover and return object', function () {
      expect(getTurnover({ hoursPerYear, hourRate })).toEqual({
  ...
  describe('calculateSurplus', () => {
    ...
    it('should calculate surplus when true', () => {
      expect(calculateSurplus(income, { surplus: { amount, include: true } })).toEqual({
        after: income - amount,
        surplus: amount,
      });
    });
    it('should not calculate surplus when false', () => {
      expect(
        calculateSurplus(income, {
          surplus: { amount, include: false },
        })
  ...
  describe('getEmployerFee', () => {
    it('should calculate the employer fee when included', () => {
      expect(
        getEmployerFee(expectedAfterCut, {
          employerFee: { include: true, rate: 14.1 },
        })
    ...
    it('should not calculate the employer fee when not included', () => {
      expect(
        getEmployerFee(expectedAfterCut, {
          employerFee: { include: false, rate: 14.1 },
        })
  ...
  describe('getVacationSavings', () => {
    it('should calculate the vacation savings when included', () => {
      expect(
        getVacationSavings(expectedAfterEmployerFee, {
          vacationSavings: { include: true, rate: 12.5 },
        })
      ).toEqual({
    ...
    it('should not calculate the vacation savings when not included', () => {
      expect(
        getVacationSavings(expectedAfterEmployerFee, {
          vacationSavings: { include: false, rate: 12.5 },
        })
